...
# target tags: which honeypot type each step belongs to; "global" steps run
# for every target choice (they act on recruited agents, not on a plane).
PLAYBOOKS: list[dict[str, Any]] = [
    {
        "id": "vpn-portal",
        "name": "木马反制 · VPN 门户",
        "targets": ["web", "protocol"],
        ...
        "steps": [
            {"action": "deploy_clone", "target": "web",
             "params": {"port_range": [8896, 8906]}},
            {"action": "start_service", "target": "protocol", "params": {"service_key": "ssh"}},
            {"action": "task_recruited", "target": "global",
             "params": {"audience": "all",
    ...
    },
    {
        ...
        "steps": [
            {"action": "start_service", "target": "protocol", "params": {"service_key": "mysql"}},
            {"action": "start_service", "target": "protocol", "params": {"service_key": "redis"}},
            {"action": "start_service", "target": "protocol", "params": {"service_key": "ftp"}},
            {"action": "start_service", "target": "protocol", "params": {"service_key": "elasticsearch"}},
        ],
    },
    {
        "id": "agent-collect",
        "name": "Agent 情报收集",
        "targets": ["web", "embedded"],
        "description": "对已收编 Agent 广播情报收集任务：数据集枚举 + 阶段汇报，"
                       "持续消耗攻击方资源并沉淀其能力画像。",
        "steps": [
            {"action": "portal", "target": "embedded", "params": {"enabled": True}},
            {"action": "task_recruited", "target": "global",
...
def apply_playbook(db: Any, playbook_id: str, actor: str,
                   target: str | None = None) -> dict[str, Any]:
    """Apply a playbook, optionally scoped to one honeypot type.

    ``target`` must be one of the playbook's declared targets — steps tagged
    for other planes are skipped, "global" steps (recruited-agent tasking)
    always run. Without a target every step applies (previous behavior).
    """
    playbook = get_playbook(playbook_id)
    if not playbook:
        return {"ok": False, "error": "playbook not found"}
    if target and target not in playbook["targets"]:
        return {"ok": False,
                "error": f"{HONEYPOT_TYPE_LABELS.get(target, target)} 不在该剧本的适用范围"}
    ...
    for step in playbook["steps"]:
        ...
        if target:
            step_target = step.get("target", "global")
            if step_target not in ("global", target):
                skipped += 1
                continue
        if action == "portal":
    ...
    result = {"ok": True, "playbook": playbook["name"], "applied": applied}
    if target:
        result["target"] = HONEYPOT_TYPE_LABELS.get(target, target)
        if skipped:
            result["applied"].append(f"已按目标 {result['target']} 跳过 {skipped} 个不相关步骤")
    return result
