...
NAV_ICONS = {
    ...
    "/admin/api-tokens": "第三方威胁数源接入与开放 API Key 管理",
}
...
NAV_DESCRIPTIONS = {
    ...
    "/admin/counter-offense/behavior": "会话行为自动化特征分析参数",
...
def admin_services(request: Request, db: Session = Depends(get_db)):
    ...
    return _render(
        ...
        "admin/services.html",
        {
            ...
            "error": request.query_params.get("error", ""),
            "playbooks": __import__("app.services.playbooks", fromlist=["get_playbooks"]).get_playbooks(),
...
@router.post("/admin/services/playbook/apply")
def apply_playbook_from_services(
    playbook_id: str = Form(""),
    target: str = Form(""),
    request: Request = None,
    db: Session = Depends(get_db),
):
    """Apply a 反制剧本 straight from 端口服务蜜罐管理's 操作一览."""
    user = _require_admin(request, db)
    from app.services.playbooks import apply_playbook

    if not playbook_id:
        return _redirect("/admin/services" + _qs(error="请选择要应用的反制剧本"))
    ...
    if result.get("ok"):
        summary = "；".join(result.get("applied", []))
        return _redirect(f"/admin/services{_qs(applied=result.get('playbook', playbook_id) + '：' + summary)}")
    return _redirect(f"/admin/services{_qs(error=str(result.get('error')))}")
...
async def config_metadata_face(
    request: Request,
    db: Session = Depends(get_db),
):
    """云元数据蜜罐（SSRF）config — lives on the Web应用蜜罐管理 page; storage
    is the shared metadata surface so the deployment page stays in sync."""
    user = _require_admin(request, db)
    from app.services.surface_config import set_surface

    form = await request.form()
    config = {
        "role": (form.get("role") or "").strip() or "prod-app-role",
        "account_id": (form.get("account_id") or "").strip() or "100000000001",
    }
    set_surface(
        db,
        key="metadata",
...
async def config_dataset_face(request: Request, db: Session = Depends(get_db)):
    """无限资源消耗数据集 — configured on the Web应用蜜罐管理 page."""
    user = _require_admin(request, db)
    from app.services.surface_config import set_surface

    form = await request.form()
    config = {
        "total_pages": int(form.get("total_pages") or 999999),
        "rows_per_page": int(form.get("rows_per_page") or 10),
        "email_domain": (form.get("email_domain") or "").strip() or "corp-internal.example",
        "note_prefix": (form.get("note_prefix") or "").strip() or "内部核对",
    }
    set_surface(db, key="dataset", enabled=form.get("enabled") == "on",
                actor=user.username, config=config)
...
async def config_intranet_face(request: Request, db: Session = Depends(get_db)):
    """内网横向 Wiki — configured on the Web应用蜜罐管理 page."""
    user = _require_admin(request, db)
    from app.services.surface_config import set_surface

    form = await request.form()
    hosts = []
    for line in (form.get("hosts") or "").splitlines():
        line = line.strip()
    ...
    config = {
        "title": (form.get("title") or "").strip() or "内部知识库 · Corp Wiki",
        "duty": (form.get("duty") or "").strip() or "wei.chen / fang.wang",
    }
    if hosts:
        config["hosts"] = hosts
    set_surface(db, key="intranet", enabled=form.get("enabled") == "on",
                actor=user.username, config=config)
    log_execution(db, actor_username=user.username, action="update",
                  module="templates", target_type="web-honeypot-config",
                  target_ref="intranet",
...
async def config_portal_face(request: Request, db: Session = Depends(get_db)):
    """功能性伪装反制（Portal）— configured on the Web应用蜜罐管理 page;
    storage is portal_config so the standalone page (kept for direct links)
    and the deployment page stay in sync."""
    user = _require_admin(request, db)
    from app.services.portal_config import get_config_row, save_config
    ...
    row = get_config_row(db)
    try:
        heartbeat = int(form.get("heartbeat_interval") or row.heartbeat_interval or 30)
        register_max = int(form.get("register_max_per_ip_hour") or row.register_max_per_ip_hour or 20)
    except (TypeError, ValueError):
        heartbeat, register_max = row.heartbeat_interval, row.register_max_per_ip_hour
    save_config(
        db,
        actor=user.username,
        enabled=form.get("enabled") == "on",
        footer_enabled=form.get("footer_enabled") == "on",
        footer_title=(form.get("footer_title") or "").strip() or "Developer API",
...
def admin_templates(request: Request, db: Session = Depends(get_db)):
    ...
    metadata_face = {
        "enabled": md_runtime.get("enabled", True),
        "config": md_runtime.get("config", {}),
        "hits_24h": surface_stats(db).get("metadata", {}).get("hits_24h", 0),
    }
    ...
    def _face_ctx(key: str) -> dict:
        ...
        return {
            "enabled": rt.get("enabled", True),
            "config": rt.get("config", {}),
            "hits_24h": surface_stats(db).get(key, {}).get("hits_24h", 0),
        }
...
def admin_decoy_management(request: Request, db: Session = Depends(get_db)):
    ...
    for item in templates_list:
        ...
        # a bound template may have been deleted since binding — degrade to
        # "未绑定" instead of crashing the whole page
        route_tpl = template_lookup.get(item.bind_route_template_id)
        cred_tpl = template_lookup.get(item.bind_credential_template_id)
        item.bound_route_name = route_tpl.name if route_tpl else ""
        item.bound_credential_name = cred_tpl.name if cred_tpl else ""
...
@router.post("/admin/api-tokens/providers/{provider_key}")
async def save_intel_provider(
    provider_key: str,
    request: Request,
    db: Session = Depends(get_db),
):
    """Generic provider save: dispatch by registry key (威胁数源模板)."""
    user = _require_admin(request, db)
    from app.services.intel_providers import get_provider

    provider = get_provider(provider_key)
    if provider is None:
        return _redirect(f"/admin/api-tokens{_qs(intel_err='未知的数源类型')}")

    form = await request.form()
    if provider_key == "threatbook":
        from app.services import threatbook

        # empty input keeps the stored key (the form shows it masked)
        api_key = (form.get("api_key") or "").strip() or threatbook.get_api_key()
        enabled = form.get("enabled") == "on"
        if enabled and not api_key:
            return _redirect(f"/admin/api-tokens{_qs(intel_err='启用微步在线需要先填写 API Key')}")
        threatbook.save_config(db, api_key=api_key, enabled=enabled, actor=user.username)
        log_execution(
            db,
            actor_username=user.username,
            action="update",
            module="intel",
            target_type="provider-config",
            target_ref=provider_key,
            detail_json={"enabled": enabled, "key_set": bool(api_key)},
        )
        label = provider["short"]
        return _redirect(f"/admin/api-tokens{_qs(saved=label + ' 接入配置已保存')}")
    return _redirect(f"/admin/api-tokens{_qs(intel_err='该数源暂未实现保存逻辑')}")

@router.post("/api/admin/api-tokens/threatbook/lookup")
async def api_threatbook_lookup(request: Request, db: Session = Depends(get_db)):
    user = _require_admin(request, db)
    from app.services import threatbook
    ...
    return JSONResponse(threatbook.lookup_ip(ip, refresh=refresh))
...
def admin_api_tokens(request: Request, raw_token: str | None = None, db: Session = Depends(get_db)):
    user = _require_user(request, db)
    from app.services.intel_providers import list_providers, provider_status

    providers = [
        {"meta": meta, "status": provider_status(db, meta["key"])}
    ...
    return _render(
        ...
        "admin/api_tokens.html",
        {
            ...
            "providers": providers,
            "saved": request.query_params.get("saved", ""),
            "intel_err": request.query_params.get("intel_err", ""),
        },
    )
...
def admin_playbooks(request: Request, db: Session = Depends(get_db)):
    ...
    return _render(
        ...
        {"title": "反制剧本", "playbooks": get_playbooks(),
         "honeypot_type_labels": __import__("app.services.playbooks", fromlist=["HONEYPOT_TYPE_LABELS"]).HONEYPOT_TYPE_LABELS,
         "applied": request.query_params.get("applied", ""),
         "user": user},
    )
...
async def api_managed_host_note(
    ...
    # copy: assigning the SAME dict back is a no-op for SQLAlchemy change
    # detection and the note silently never persisted
